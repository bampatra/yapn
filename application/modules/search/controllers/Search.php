...
class Search extends MX_Controller
{
    ...
    function get_all_products(){
        ...
        if(isset($_SESSION['id_web_user']) && isset($_REQUEST['activity'])) {
            // add to activity
            $this->add_to_activity($_SESSION['id_web_user'], htmlentities($_REQUEST['q'], ENT_QUOTES));
        }
        ...
        $data = $this->Search_model->get_all_products(htmlentities($_REQUEST['q'], ENT_QUOTES), $id_web_user, $filters, $group_by, $sort_by, $limit, $offset);
    ...
    private function add_to_activity($id_web_user, $search){

        date_default_timezone_set('Asia/Jakarta');

        // check if activity exists in database
        $check = $this->Profile_model->get_activity_plain($id_web_user, 'SEARCH', $search);
        ...
        if($check->num_rows() > 0){
            ...
            $activity_data = array(
                ...
                'tipe_web_user_activity' => 'SEARCH',
                'detail_web_user_activity' => $search,
    ...
    function get_max_min(){
        $data = $this->Search_model->get_all_products(htmlentities($_REQUEST['q'], ENT_QUOTES), 0);
