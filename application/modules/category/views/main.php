...
<script>
    ...
    $.ajax({
        ...
        url         : category_url + 'get_category_name', // the url where we want to POST
        data        : {cat: urlParams.get('cat')}, // our data object
        dataType    : 'json',
        success     : function(data){
            if(data.length > 0){
                $('.nama_kategori').html('<h2>'+ data[0].nama_web_catprod +'</h2>');
            } else {
                $('.nama_kategori').html('<h2> null </h2>');
            }
    ...
    function loadmore(){
        if(load){
            ...
            $.ajax({
                type        : 'POST', // define the type of HTTP verb we want to use (POST for our form)
                url         : category_url + 'get_all_products', // the url where we want to POST
                data        : $('form').serialize() + "&cat=" + urlParams.get('cat') + "&limit=" + limit + "&offset=" + offset, // our data object
                dataType    : 'json',
                success     : function(data){
                    if(data.length > 0){
    ...
    function filter_load(){
        $.ajax({
            type: 'POST', // define the type of HTTP verb we want to use (POST for our form)
            url: category_url + 'get_all_products', // the url where we want to POST
            data: {cat: urlParams.get('cat'), group_by: 'color', 'activity': true}, // our data object
            dataType: 'json',
            success: function (data) {
    ...
    $.ajax({
        type        : 'POST', // define the type of HTTP verb we want to use (POST for our form)
        url         : category_url + 'get_all_products', // the url where we want to POST
        data        : {cat: urlParams.get('cat'), limit: limit, offset: offset}, // our data object
        dataType    : 'json',
        success     : function(data){
            mainLoad(data);
        }
    ...
    $.ajax({
        type        : 'POST', // define the type of HTTP verb we want to use (POST for our form)
        url         : category_url + 'get_max_min', // the url where we want to POST
        data        : {cat: urlParams.get('cat')}, // our data object
        dataType    : 'json',
        success     : function(data){
